---
title: "Markdown Tutorial: Van WYSIWYG naar Platte Tekst"
abstract: |
  Markdown is een eenvoudige opmaaktaal voor platte tekst die steeds populairder wordt. In dit artikel lees je wat Markdown is, waarom het handig is, wanneer je het moet gebruiken, en hoe je snel aan de slag kunt gaan. Of je nu een programmeur, schrijver of student bent, deze gids helpt je de overstap te maken van traditionele WYSIWYG-editors naar de eenvoud en kracht van Markdown.
date: "2026-01-12"
author: "Ben Welman"
image: markdown.png
lang: nl
...
# Wiskundige notatie en formules

::: callout-important
Standaard Markdown heeft geen ingebouwde ondersteuning voor wiskundige formules. Voor wiskundige notatie moet je gebruikmaken van **LaTeX-syntax**, in combinatie met een rendering-engine zoals MathJax of KaTeX.

**Goed nieuws:** Nagenoeg alle Markdown-editors en platforms ondersteunen LaTeX-wiskunde zoals Obsidian, Typora, Jupyter Notebooks, Quarto, GitHub, GitLab, Notion, VS Code, ...
:::

**Hoe werkt het?**

Voor **symbolen** en **inline formules** in lopende tekst gebruik je **enkele dollar-tekens** met daartussen de uit drukking.

`De beroemde formule van Einstein is $E = mc^2$`

Wordt weergegeven als: De beroemde formule van Einstein is $E = mc^2$

Voor **display formules** (op een aparte regel en gecentreerd) gebruik je **dubbele dollar-tekens** met daartussen de uitdrukking.
...
Latex symbolen beginnen altijd met een `\`. Zo wordt `$\alpha$` weergegeven als $\alpha$, en wordt `$\sqrt{2}$` weergegeven als $\sqrt{2}$, en het sommatieteken `$\sum$` wordt $\sum$.

Een deling kun je maken met `$\frac{teller}{noemer}$`. Bijvoorbeeld:

`$\tan(\beta) = \frac{\sin(\beta)}{\cos(\beta)}$` wordt $\tan(\beta) = \frac{\sin(\beta)}{\cos(\beta)}$

Er zijn talloze sites waarop je overzichten van Latex symbolen en formules vindt, bijvoorbeeld [wikibooks](https://nl.wikibooks.org/wiki/LaTeX/Wiskundige_formules).

# Wat is lastig of moeilijk?

## **1.** Afbeeldingen positioneren

In Word kun je een afbeelding precies plaatsen en tekst eromheen laten lopen. In Markdown is dit standaard niet mogelijk. Afbeeldingen staan op hun eigen regel.
...
**Oplossing:** Dit went snel, na een paar documenten wordt dit automatisme.
...
Markdown is niet de oplossing voor alle tekstverwerkingsbehoeften, maar het is een waardevol gereedschap in je digitale toolbox.
...
[Dit artikel in markdown formaat](_markdowngids.md)
